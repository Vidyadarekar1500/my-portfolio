  ...
  return (
    <div className="flex justify-between items-center text-white fixed bg-black w-full h-20 px-4">
      ...
      <div className="md:hidden">
        ...
        {isOpenNav && <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500 z-[-1]">
          {navItems.map(({ id, link }) => (
            <li
              key={id}
            ...
            >
            <Link onClick={() => setIsOpenNav(!isOpenNav)} to={link} smooth={true} duration={500}>{link}</Link>
            </li>
          ))}
        </ul>}
      </div>
